...
- ⏳ **Queued** — planned for its own dedicated batch

| Category | Area | Status | Notes |
|----------|------|--------|-------|
| Food & drink | Sweet Martha's | ✅ | Wanted a couple; only one banked so far. |
...
| A second Sweet Martha's angle | Parked | Wanted "a couple" of Martha squares; only one banked. |
...
- **Center-square candidates** — ✅ ten landed (ids C1–C10, `centers` array in [`src/ts/partials/squares/_centers.ts`](../src/ts/partials/squares/_centers.ts)): a white person, something fried, food on a stick, Crocs, Twins merch, a flag, corn, trash, something with cheese, daytime drinking. All `gimme`/`see`. The joke is inevitability ("of course you did"), so they stay flat and unadorned. Treatment decided: **player-marked** (issue #14). Centers use the `C` id prefix, guarded across all groups by `tests/ids.test.ts`.
